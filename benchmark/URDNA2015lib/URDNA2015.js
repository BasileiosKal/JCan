...
module.exports = class URDNA2015 {
  ...
  async main(dataset) {
    ...
    for(const id of nonNormalized) {
      ...
      const nquads = await this._hashAndTrackBlankNode({id, hashToBlankNodes});
      this.NQuads_List.push(nquads);
    }
    ...
    return [normalized.join(''), this.NQuads_List];
  }
  ...
  async hashFirstDegreeQuads(id) {
    ...
    // console.log("URDNA2015: hashFirstDegreeQuads: Nquads Hash: ");
    // console.log(info.hash)
    return [info.hash, nquads];
  }
  ...
  async _hashAndTrackBlankNode({id, hashToBlankNodes}) {
    ...
    // Quads algorithm.
    const returnedValues = await this.hashFirstDegreeQuads(id);
    const hash = returnedValues[0];
    const nquads = returnedValues[1];
